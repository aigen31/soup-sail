<?php

namespace App\Livewire\Admin\Users;

use App\Models\Project;
use App\Models\User;
use App\View\Components\Layers\ControlLayout;
use App\View\Support\Content\Description;
use App\View\Support\SortTable;
use Livewire\Component;
use Livewire\WithPagination;

class UserProject extends Component
{
  use WithPagination;
  use SortTable;

  public $projectId;

  public function sortBy(): string
  {
    switch ($this->sortBy) {
      case '0':
        return 'created_at';
        break;
      case '1':
        return 'updated_at';
        break;
      case '2':
        return 'name';
        break;
    }
  }

  public function getProjectProperty()
  {
    return Project::find($this->projectId);
  }

  public function getTasksProperty()
  {
    $tasks = Project::find($this->projectId)->tasks()->orderBy($this->sortBy(), $this->sortType)->paginate(10);

    foreach ($tasks as $item) {
      $item->description = Description::shortString($item->description);
    }

    return $tasks;
  }

  public function render()
  {
    return view('livewire.admin.users.user-project')->layout(ControlLayout::class);
  }
}
